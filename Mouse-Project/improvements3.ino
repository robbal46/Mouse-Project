...
void setup() {
  ...
  distanceError = (0.07021*totalError) - 1.267;
...
bool overLine(){
  ...
  if (!straight && inductorFValue >= 450){
    startStraight = millis();
    straight = true;
  }
  else if (inductorFValue <= 450){
    straight = false;
  }
...
void setMotorSpeeds(){
  ...
  if(speedUp && millis()<1000 || millis()>6000){
    motorLSpeed = boostSpeed - motorChange;
    motorRSpeed = boostSpeed + motorChange;
    sensorPID.SetTunings(0.9,0,0);
    sensorPID.SetOutputLimits((-1 * boostSpeed),255-boostSpeed);
  }  
  else if (inductorFValue <= 180 && distanceError < 0 && millis() > 4600 && millis() < 6000){
    motorLSpeed = 0;//slowSpeed - motorChange;
    motorRSpeed = baseSpeed + motorChange;
    sensorPID.SetOutputLimits((-1*baseSpeed), 255-baseSpeed);
